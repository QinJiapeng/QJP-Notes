...
![](https://img.jiapeng.store/img/202307081701506.png)
...
![](https://img.jiapeng.store/img/202307081701507.png)
...
* Mapper.xml 文件中的增删改查标签的 resultType 属性和 DAO 层 Mapper 接口方法的返回值相同
  ...
  ![](https://img.jiapeng.store/img/202307081701508.png)
...
![](https://img.jiapeng.store/img/202307081701510.png)
...
<img src="https://img.jiapeng.store/img/202307081701511.png" style="zoom: 67%;" />
...
![](https://img.jiapeng.store/img/202307081701512.png)
...
![](https://img.jiapeng.store/img/202307081701513.png)
  ...
  * `Configuration.addMappers()`：将核心配置文件配置的映射器添加到 mapperRegistry 中，用来**获取代理对象**
    ...
    * `parser.parse()`：解析 Mapper 接口
      ...
      * `SqlSource sqlSource = getSqlSourceFromAnnotations()`：获取 SQL 的资源对象
        ...
        ![](https://img.jiapeng.store/img/202307081701514.png)
...
![](https://img.jiapeng.store/img/202307081701515.png)
...
![](https://img.jiapeng.store/img/202307081701516.png)
...
![](https://img.jiapeng.store/img/202307081701517.png)
...
* `CachingExecutor.query()`：先执行 CachingExecutor 去二级缓存获取数据
  ...
  * `MappedStatement.getBoundSql(parameterObject)`：**把 parameterObject 封装成 BoundSql**
    ...
    ![](https://img.jiapeng.store/img/202307081701518.png)
...
![](https://img.jiapeng.store/img/202307081701519.png)
...
<img src="https://img.jiapeng.store/img/202307081701520.png" style="zoom:50%;" />
...
![](https://img.jiapeng.store/img/202307081701521.png)
...
![](https://img.jiapeng.store/img/202307081701522.png)
...
![](https://img.jiapeng.store/img/202307081701523.png)
...
![](https://img.jiapeng.store/img/202307081701524.png)
...
5. 表现层（App）通过 Spring 获取资源（Service 实例）
   ...
   ![](https://img.jiapeng.store/img/202307081701525.png)
...
- DI（Dependency Injection）依赖注入，应用程序运行依赖的资源由 Spring 为其提供，资源进入应用程序的方式称为注入，简单说就是利用反射机制为类的属性赋值的操作
  ...
  ![](https://img.jiapeng.store/img/202307081701526.png)
...
![](https://img.jiapeng.store/img/202307081701527.png)
...
![](https://img.jiapeng.store/img/202307081701528.png)
...
![](https://img.jiapeng.store/img/202307081701529.png)

![](https://img.jiapeng.store/img/202307081701530.png)

![](https://img.jiapeng.store/img/202307081701531.png)
...
* 流程图：![](https://img.jiapeng.store/img/202307081701532.png)
...
* 流程图：![](https://img.jiapeng.store/img/202307081701533.png)
...
![](https://img.jiapeng.store/img/202307081701534.png)
...
![](https://img.jiapeng.store/img/202307081701535.png)
...
![AOP织入时机](https://img.jiapeng.store/img/202307081701536.png)
...
![](https://img.jiapeng.store/img/202307081701537.png)
...
![](https://img.jiapeng.store/img/202307081701538.png)
...
- 数据层：负责数据操作
  ...
  ![](https://img.jiapeng.store/img/202307081701539.png)
...
* Controller（控制器）：处理用户交互的调度器，用于根据用户需求处理程序逻辑
  ...
  * SpringMVC
  ...
  ![](https://img.jiapeng.store/img/202307081701540.png)
...
![](https://img.jiapeng.store/img/202307081701541.png)
...
* View：视图， View 最后对页面进行渲染将结果返回给用户，SpringMVC 框架提供了很多的 View 视图类型，包括：jstlView、freemarkerView、pdfView 等
  ...
  ![](https://img.jiapeng.store/img/202307081701542.png)
...
![](https://img.jiapeng.store/img/202307081701543.png)
...
![](https://img.jiapeng.store/img/202307081701544.png)
  ...
  * `Object handler = getHandlerInternal(request)`：**获取映射器**，底层调用 RequestMappingInfoHandlerMapping 类的方法，又调用 AbstractHandlerMethodMapping#getHandlerInternal
    ...
    * `handlerMethod = lookupHandlerMethod(lookupPath, request)`：获取当前 HandlerMapping 中的映射规则
      ...
      * `directPathMatches = this.mappingRegistry.getMappingsByDirectPath(lookupPath)`：获取当前的映射器与当前**请求的 URI 有关的所有映射规则**
        ...
        ![](https://img.jiapeng.store/img/202307081701545.png)
...
![](https://img.jiapeng.store/img/202307081701546.png)
...
* `writeWithMessageConverters(returnValue, returnType, inputMessage, outputMessage)`：使用消息转换器进行写出
  ...
  * `acceptableTypes = getAcceptableMediaTypes(request)`：**获取浏览器支持的媒体类型，请求头字段**
  ...
  ![](https://img.jiapeng.store/img/202307081701547.png)
...
* `genericConverter.write(body, targetType, selectedMediaType, outputMessage)`：调用消息转换器的 write 方法
  ...
  * `addDefaultHeaders(headers, t, contentType)`：**设置响应头中的数据类型**
    ...
    ![](https://img.jiapeng.store/img/202307081701548.png)
...
2. 拦截内容不同： Filter 对所有访问进行增强， Interceptor 仅针对 SpringMVC 的访问进行增强  
   ...
   <img src="https://img.jiapeng.store/img/202307081701549.png" style="zoom:67%;" />
...
<img src="https://img.jiapeng.store/img/202307081701550.png" style="zoom:67%;" />
...
<img src="https://img.jiapeng.store/img/202307081701551.png" style="zoom: 50%;" />
...
![](https://img.jiapeng.store/img/202307081701552.png)
...
* 三种判定空校验器的区别 
  ![](https://img.jiapeng.store/img/202307081701553.png)
...
![](https://img.jiapeng.store/img/202307081701554.png)
...
* **@EnableAutoConfiguration 注解：启用 SpringBoot 的自动配置机制**
  ...
  * 从 spring-boot-autoconfigure-2.5.3.jar/META-INF/spring.factories 文件中寻找 EnableAutoConfiguration 字段，获取自动装配类，**进行条件装配，按需装配**
    ...
    ![](https://img.jiapeng.store/img/202307081701555.png)
...
* **profile 激活方式**
  ...
  * 虚拟机参数：在VM options 指定：`-Dspring.profiles.active=dev`
    ...
    ![](https://img.jiapeng.store/img/202307081701556.png)
...
![Cloud-组件概览](https://img.jiapeng.store/img/202307081701557.png)
...
![Cloud-Eureka和Dubbo对比](https://img.jiapeng.store/img/202307081701558.png)
...
* 浏览器访问 http://localhost:7001
  ...
  ![Cloud-Eureka可视化界面](https://img.jiapeng.store/img/202307081701559.png)
...
![Cloud-Eureka集群原理](https://img.jiapeng.store/img/202307081701560.png)
...
* 访问 http://eureka7001.com:7001 和 http://eureka7002.com:7002：
  ...
  ![Cloud-EurekaServer集群构建成功](https://img.jiapeng.store/img/202307081701561.png)
...
![Cloud-Eureka自我保护机制](https://img.jiapeng.store/img/202307081701562.png)
...
![Cloud-Ribbon架构原理](https://img.jiapeng.store/img/202307081701563.png)
...
![Cloud-IRule类图](https://img.jiapeng.store/img/202307081701564.png)
...
* 测试报错：
  ...
  ![Cloud-OpenFeign超时错误](https://img.jiapeng.store/img/202307081701565.png)!](C:\Users\Seazean\Desktop\123\Cloud-OpenFeign超时错误.png)
...
![Cloud-Hystrix熔断机制](https://img.jiapeng.store/img/202307081701566.png)
...
![Cloud-Hystrix工作流程](https://img.jiapeng.store/img/202307081701567.png)
...
* 启动测试：http://localhost:9001/hystrix
  ...
  <img src="https://img.jiapeng.store/img/202307081701568.png" alt="Cloud-Hystrix可视化界面" style="zoom: 67%;" />
...
* 指标说明：
  ...
  ![Cloud-Hystrix界面图示说明](https://img.jiapeng.store/img/202307081701569.png)
...
![Cloud-Gateway工作流程](https://img.jiapeng.store/img/202307081701570.png)
...
* 测试：正常访问成功，将时间修改到 2023-01-10T16:31:44.106+08:00[Asia/Shanghai] 之后访问失败
  ...
  ![Cloud-Gateway时间断言](https://img.jiapeng.store/img/202307081701571.png)
...
![Cloud-Config工作原理](https://img.jiapeng.store/img/202307081701572.png)
...
<img src="https://img.jiapeng.store/img/202307081701573.png" alt="Cloud-Bus全局广播架构" style="zoom:67%;" />
...
![Cloud-Bus工作流程](https://img.jiapeng.store/img/202307081701574.png)
...
![Cloud-Stream工作流程](https://img.jiapeng.store/img/202307081701575.png)
...
* 管理后台服务：
  ...
  ![Cloud-Nacos服务列表](https://img.jiapeng.store/img/202307081701576.png)
...
* 新增配置，然后访问 http://localhost:3377/config/info
  ...
  ![Cloud-Nacos新增配置](https://img.jiapeng.store/img/202307081701577.png)
...
![Cloud-Nacos配置说明](https://img.jiapeng.store/img/202307081701578.png)
...
![Cloud-Nacos命名空间](https://img.jiapeng.store/img/202307081701579.png)
...
<img src="https://img.jiapeng.store/img/202307081701580.png" alt="Cloud-Sentinel增加流控规则" style="zoom: 40%;" />
...
<img src="https://img.jiapeng.store/img/202307081701581.png" alt="Cloud-Sentinel增加熔断规则" style="zoom: 67%;" />
...
<img src="https://img.jiapeng.store/img/202307081701582.png" alt="Cloud-Sentinel热点参数限流" style="zoom: 67%;" />
...
<img src="https://img.jiapeng.store/img/202307081701583.png" alt="Cloud-Sentinel增加热点规则" style="zoom:50%;" />
...
<img src="https://img.jiapeng.store/img/202307081701584.png" alt="Cloud-Sentinel增加系统规则" style="zoom:50%;" />
...
<img src="https://img.jiapeng.store/img/202307081701585.png" alt="Cloud-分布式事务流程" style="zoom:67%;" />
...
*  file.conf：自定义事务组名称、事务日志存储模式为 db、数据库连接信息
  ...
  **事务分组**：seata 的资源逻辑，可以按微服务的需要，在应用程序（客户端）对自行定义事务分组，每组取一个名字
  ...
  <img src="https://img.jiapeng.store/img/202307081701586.png" alt="Cloud-Seata配置文件" style="zoom:50%;" />
...
* registry.conf：指明注册中心为 Nacos，及修改 Nacos 连接信息
  ...
  <img src="https://img.jiapeng.store/img/202307081701587.png" alt="Cloud-Seata注册中心配置" style="zoom: 71%;" />

启动 Nacos 和 Seata，如果 DB 报错，需要把将 lib 文件夹下 mysql-connector-java-5.1.30.jar 删除，替换为自己 MySQL 连接器版本

<img src="https://img.jiapeng.store/img/202307081701589.png" alt="Cloud-Seata配置成功" style="zoom:80%;" />
