...
<img src="https://img.jiapeng.store/img/202307051621762.png" style="zoom: 80%;" />
...
<img src="https://img.jiapeng.store/img/202307051621763.png" style="zoom: 80%;" />
...
* Java1.8 以前的内存结构图：
  ![](https://img.jiapeng.store/img/202307051621765.png)

* Java1.8 之后的内存结果图：

  ![](https://img.jiapeng.store/img/202307051621766.png)
...
* 每个栈由多个栈帧（Frame）组成，对应着每次方法调用时所占用的内存，每个栈帧中存储着：
  ...
  * 操作数栈或表达式栈和其他一些附加信息
  ...
  <img src="https://img.jiapeng.store/img/202307051621767.png" style="zoom:50%;" />
...
* 为了支持当前方法的代码能够实现动态链接，每一个栈帧内部都包含一个指向运行时常量池或该栈帧所属方法的引用
  ...
  ![](https://img.jiapeng.store/img/202307051621768.png)

* 在 Java 源文件被编译成的字节码文件中，所有的变量和方法引用都作为符号引用保存在 class 的常量池中
  
  常量池的作用：提供一些符号和常量，便于指令的识别
  
  ![](https://img.jiapeng.store/img/202307051621769.png)
...
<img src="https://img.jiapeng.store/img/202307051621770.png" style="zoom:67%;" />
...
<img src="https://img.jiapeng.store/img/202307051621771.png" style="zoom: 67%;" />
...
![](https://img.jiapeng.store/img/202307051621772.jpg)
...
![](https://img.jiapeng.store/img/202307051621773.jpg)
...
<img src="https://img.jiapeng.store/img/202307051621774.png" style="zoom: 67%;" />
...
![](https://img.jiapeng.store/img/202307051621775.png)
...
- 在可达性分析算法中，只有能够被根对象集合直接或者间接连接的对象才是存活对象
  ...
  <img src="https://img.jiapeng.store/img/202307051621776.png" style="zoom: 50%;" />
...
<img src="https://img.jiapeng.store/img/202307051621777.gif" style="zoom: 67%;" />
...
<img src="https://img.jiapeng.store/img/202307051621778.png" style="zoom: 50%;" />
...
<img src="https://img.jiapeng.store/img/202307051621779.png" style="zoom:50%;" />
...
![](https://img.jiapeng.store/img/202307051621780.png)
...
<img src="https://img.jiapeng.store/img/202307051621781.png" style="zoom: 67%;" />
...
<img src="https://img.jiapeng.store/img/202307051621782.png" style="zoom:67%;" />
...
![](https://img.jiapeng.store/img/202307051621783.png)
...
![](https://img.jiapeng.store/img/202307051621784.png)
...
![](https://img.jiapeng.store/img/202307051621785.png)
...
![](https://img.jiapeng.store/img/202307051621786.png)
...
![](https://img.jiapeng.store/img/202307051621787.png)
...
![](https://img.jiapeng.store/img/202307051621788.png)
...
<img src="https://img.jiapeng.store/img/202307051621789.png" style="zoom:67%;" />
...
<img src="https://img.jiapeng.store/img/202307051621790.png" style="zoom: 50%;" />
...
* **Concurrent Mark **：
  ...
  * 筛选回收：并发清理阶段，首先对 CSet 中各个 Region 中的回收价值和成本进行排序，根据用户所期望的 GC 停顿时间来制定回收计划，也需要 STW
  ...
  ![](https://img.jiapeng.store/img/202307051621791.jpg)
...
![](https://img.jiapeng.store/img/202307051621792.png)
...
<img src="https://img.jiapeng.store/img/202307051621793.png" style="zoom: 67%;" />
...
![](https://img.jiapeng.store/img/202307051621794.png)
...
* 句柄访问：Java 堆中会划分出一块内存来作为句柄池，reference 中存储的就是对象的句柄地址，而句柄中包含了对象实例数据和类型数据各自的具体地址信息
  ...
  ![](https://img.jiapeng.store/img/202307051621795.png)
...
* 直接指针（HotSpot 采用）：Java 堆对象的布局必须考虑如何放置访问类型数据的相关信息，reference 中直接存储的对象地址
  ...
  ![](https://img.jiapeng.store/img/202307051621796.png)
...
![](https://img.jiapeng.store/img/202307051621797.png)
...
<img src="https://img.jiapeng.store/img/202307051621798.png" style="zoom:80%;" />
...
<img src="https://img.jiapeng.store/img/202307051621799.png" style="zoom: 50%;" />
...
<img src="https://img.jiapeng.store/img/202307051621800.png" style="zoom: 33%;" />
...
<img src="https://img.jiapeng.store/img/202307051621801.png" style="zoom:67%;" />
...
![](https://img.jiapeng.store/img/202307051621802.png)
...
<img src="https://img.jiapeng.store/img/202307051621803.png" style="zoom:50%;" />
...
![](https://img.jiapeng.store/img/202307051621804.png)
...
<img src="https://img.jiapeng.store/img/202307051621805.png" style="zoom: 33%;" />
...
* **执行引擎**开始执行字节码
  ...
  * 这里小的数字都是和字节码指令存在一起，超过 short 范围的数字存入了常量池
  ...
  ![](https://img.jiapeng.store/img/202307051621806.png)

  `ldc #3`：从常量池加载 #3 数据到操作数栈
  Short.MAX_VALUE 是 32767，所以 32768 = Short.MAX_VALUE + 1 实际是在编译期间计算完成

  ![](https://img.jiapeng.store/img/202307051621807.png)
  ...
  ![](https://img.jiapeng.store/img/202307051621808.png)

  `istore_3`：将操作数栈顶数据弹出，存入局部变量表的 slot 3

  `getstatic #4`：获取静态字段

  ![](https://img.jiapeng.store/img/202307051621809.png)

  `iload_3`：

  ![](https://img.jiapeng.store/img/202307051621810.png)
  ...
  * 清除 main 操作数栈内容
  ...
  ![](https://img.jiapeng.store/img/202307051621811.png)
...
![](https://img.jiapeng.store/img/202307051621812.png)
...
<img src="https://img.jiapeng.store/img/202307051621813.png" style="zoom: 80%;" />
...
![](https://img.jiapeng.store/img/202307051621814.png)
...
![](https://img.jiapeng.store/img/202307051621815.png)
